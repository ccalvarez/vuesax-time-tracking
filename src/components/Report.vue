<template>
 ...
 mso-padding-alt:0cm 5.4pt 0cm 5.4pt"
   ...
   text-align:center;line-height:normal"
          ...
          <template v-for="(group, date) in groupedTasks" :key="date">
            <tr v-for="(task, index) in group" :key="task.id || index">
              <td
                v-if="index === 0"
...
export default {
  ...
  computed: {
    ...
    groupedTasks() {
      const grouped = {};
      if (!this.reportTasks) return grouped;

      this.reportTasks.forEach(task => {
